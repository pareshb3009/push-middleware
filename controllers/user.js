...
exports.unregister = function (req, res, callback) {
    ...
    async.waterfall([
        ...
        function (userObjectId, nextfunc) {
            ...
            request(options, function (err, httpResponse, body) {
                if (!err && httpResponse.statusCode == 200) {
                    ...
                    return nextfunc(err, httpResponse);
                }
            })
        }
    ], function (err, result) {
        if (err) {
            ...
            console.log(err);
            return res.status(result.statusCode || 500).send(err);
        }
        return res.status(result.statusCode || 200).send(result.body || "user unregistered for push notification");
    })
}
...
exports.send = function (req, res, callback) {
    ...
    Model.find({ user_id: { $in: data.tousers }, appid: data.appid }, { _id: 1 }, function (e, userArray) {
        ...
        sendMessage(resonator, title, message, messagetype, userids, appid, function (e, data={}) {
            if (e) return callback(e);
            return res.status(data.statusCode || 200).send(data.body || "push notification sent");
        });
    })
